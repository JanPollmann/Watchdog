...
=== Setup
1. Import the `Library` in your project (see instructions bellow)
...
_optional_: add a `ResultProcessor` *(this callback not monitored)* +
...
----
do not forget to add your credentials as follows in your `~/.gradle/gradle.properties`, otherwise you cannot download the library:
[source]
----
gpr.user=<YourUser>
gpr.key=<YourToken>
...
Creating a GitHub token: {url-create-github-token}
...
* For not repeated function calls, the input of a function/consumer is passed in the builder!
